...
defaults = {1,50,1e-4};% Initial guess, max iterations, tolerance.
...
LGS = logspace(0,2,520); % Can be altered for wider range or denser search.
...
idxi = abs(imag(xr)) < 5e-10;  % A very small imag term is zero.
xr(idxi) = real(xr(idxi));  % Discard small imaginary terms.
idxr = abs(real(xr)) < 5e-10;  % A very small real term is zero.
